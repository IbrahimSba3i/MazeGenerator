...
public class BestOfFourReplacement<T> implements GenerationReplacementMethod<T> {
	...
	public void createNewGeneration(List<Chromosome<T>> source, List<Chromosome<T>> target, CrossoverFunction<T> crossoverFunction) {
		...
		for(int i = 0; i < population.size(); i += 2) {
            ...
            childrenPair.add(parent1);
            childrenPair.add(parent2);
            
            Collections.sort(childrenPair);
            Collections.reverse(childrenPair);
